...
export default function NavCore({ ...props }) {
  ...
  const CSS = cn("", props.className);

  const navItems = props.items;
  const navItemCSS =
    "p-4 text-sm md:mx-1 hover:bg-ctid-gray hover:text-black active:bg-ctid-charcoal active:text-white";
      ...
      {navItems.map((item: any, idx: any) =>
        item.type === "link" ? (
          ...
          <NavList
            activeOnHover
            key={idx}
            showRef={[shownList, setShownList]}
            showKey={idx}
            className={navItemCSS}
            title={item.title}
            popout
            ...
            <div className="md:shadow-xl md:border-gray-200 border bg-white text-black">
              {item.items!.map((subItem: any, subIdx: any) => (
                <NavLink
                  ...
                  className={cn(
                    navItemCSS,
                    "pl-10 md:pl-0 md:p-5 mx-0 md:mx-0"
                  )}
                  href={subItem.link}
                  title={subItem.title}
                />
